...
| Field | Requirement |
|-------|-------------|
| `model` | Set (non-empty) as a string or an object with non-empty `name` and `source` |
| `model.llm_deployment_id` | Set **only if** `model.source` is `deployed`; otherwise absent or empty |
| `system_prompt` | Non-empty |
| `frontend.type` | Set |
| `tools` | Key **present** in YAML — either one or more tool entries, or `tools: []` |

**Deployed model rule:** the placeholder model is shared by every DataRobot-deployed LLM, so on its own it does not name one. An object with `model.source: deployed` must carry `model.llm_deployment_id` — route to [Model Selection](../SKILL.md#model-selection) when it is missing. A string `model` is always a gateway model and needs no additional model fields.
...
| `model` is an object with `source: deployed` and `llm_deployment_id` is missing or empty | Missing `model.llm_deployment_id` |
...
| Missing `model.llm_deployment_id` (deployed model, no id) | [Model Selection](../SKILL.md#model-selection) | Frontend Check → Spec Display → Agent Simulation → Post-design next steps |
